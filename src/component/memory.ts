...
export class Memory implements Component {
  ...
  init(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      logger.info(`Connecting to memory cluster ${this.conf.name} using ${this.conf.model} model`);
      ...
      switch (this.conf.model) {
        case "sentinel":
          ...
          this.client.on("error", (err) => {
            this._error_count++;
            if (this._error_count >= 60) {
              return reject("Error connecting to memory cluster");
            }
  ...
  async get(key: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.client.get(rkey, (err, data) => {
        if (err) {
          logger.error(err);
          return reject(new ApplicationError("system_error", "Error getting data from memory store", "sys_mem_get1"));
        }
